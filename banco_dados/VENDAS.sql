...
T INTO PRODUTO (CODPROD, DESCRICAO, UNIDADE, VALOR_UN) VALUES (11, 'MASCARA', 'UN ', 5);
...
ALTER TABLE VENDEDOR ADD CONSTRAINT FK_VENDEDOR_1 FOREIGN KEY (CODSETOR) REFERENCES SETOR (CODSETOR);INSERT INTO PRODUTO (CODPROD, DESCRICAO, UNIDADE, VALOR_UN) VALUES (8, 'PÁ DE PONTA', 'UN ', 70);
...
SELECT * FROM VENDEDOR where SALARIO = (select SALARIO FROM VENDEDOR where SALARIO = (select max(SALARIO) FROM VENDEDOR));

SELECT * FROM PEDIDO where NUMPED = (select NUMPED FROM ITEMPEDIDO where QTDADE = (select max(QTDADE) FROM ITEMPEDIDO));
...
select VENDEDOR.NOMEVEND,SETOR.NOMESETOR FROM VENDEDOR JOIN SETOR ON VENDEDOR.CODSETOR = SETOR.CODSETOR WHERE SETOR.NOMESETOR = "FERRAMENTAS";

select PEDIDO.CODCLI FROM PEDIDO JOIN CLIENTE ON PEDIDO.CODCLI = CLIENTE.CODCLI where CLIENTE.NOME = 'Estevan Pereira Cardoso';
